...
from energy_health_system import EnergyHealthManager
...
class GameManager:
    def __init__(self):
        ...
        from ui_simple import UIManager
        ...
        # Energy and Health System Manager
        self.energy_health_manager = EnergyHealthManager()
    ...
    def update(self):
        ...
        for snake in self.snakes[:]:  # Use slice copy to allow safe removal
            ...
            if snake.is_dead:
                continue  # Calculate basic behavior direction for snake
            ai_direction = BasicBehavior.calculate_direction(
                snake, self.food_items, self.snakes
            )

            # Determine if snake should sprint based on AI decision
            dt = 1 / 60  # Delta time for energy calculations
            is_sprinting = BasicBehavior.should_sprint(
                snake, self.food_items, self.snakes
            )
            snake.move(ai_direction, dt=dt, is_sprinting=is_sprinting)
        ...
        if len(
            self.snakes
        ) < MAX_SNAKES_ON_SCREEN and time.time() - self.last_snake_spawn_time > random.uniform(
            SNAKE_SPAWN_INTERVAL_MIN, SNAKE_SPAWN_INTERVAL_MAX
        ):
            self.spawn_snake()
            self.last_snake_spawn_time = time.time()
        ...
        self.creature_manager.update(
            self.snakes, self.food_items
        )  # Update food effects on snakes
        self.food_effects_manager.update_effects(self.snakes)

        # Update energy and health for all snakes
        dt = 1 / 60  # Assume 60 FPS for now
        for snake in self.snakes:
            if not snake.is_dead:
                # Update energy regeneration
                if hasattr(snake, "update_energy_regeneration"):
                    snake.update_energy_regeneration(
                        dt, in_territory=False, in_shelter=False
                    )
                ...
                # Add energy and health components to manager if not already present
                if snake.id not in [
                    entity_id
                    for entity_id in self.energy_health_manager.entities.keys()
                ]:
                    self.energy_health_manager.add_entity(
                        snake.id, max_energy=100, max_hp=10
                    )
    ...
    def check_collisions(self):
        ...
        for snake in self.snakes:
            ...
            for food in self.food_items[:]:  # Use slice copy for safe removal
                if snake.head_position.distance_to(food.position) < (
                    SNAKE_SEGMENT_RADIUS + food.radius
                ):
        ...
        for hunter in self.snakes:
            ...
            for prey in self.snakes:
                ...
                is_immune = any(
                    effect.effect_type == "immunity" for effect in prey_effects
                )
                ...
                if (
                    prey.size < hunter.size - FEAR_MARGIN
                    and hunter.head_position.distance_to(prey.head_position)
    ...
    def spawn_snake(self):
        ...
        print(
            f"New snake spawned with {new_snake.hp}/{new_snake.max_hp} HP for health bar testing"
        )
    ...
    def cleanup_dead_snakes(self):
        ...
        self.snakes = [
            ...
            for snake in self.snakes
            if not snake.is_dead
            or (current_time - getattr(snake, "death_time", current_time)) < 3.0
        ]
    ...
    def draw(self, screen):
        ...
        game_stats = self.ui_manager.get_game_stats(
            self.snakes, self.food_items, self.effects_manager
        )
    ...
    def draw_arcade(self, arcade_window):
        """Draw all game elements using Arcade graphics library."""
        # Draw food items
        for food in self.food_items:
            food.draw_arcade(arcade_window)

        # Draw snakes with their active effects
        for snake in self.snakes:
            # Get active effects for this snake to show visual indicators
            active_effects = self.food_effects_manager.get_snake_effects(snake.id)
            snake.draw_arcade(arcade_window, active_effects)

        # Draw creatures using the modular creature manager
        self.creature_manager.draw_arcade(arcade_window)

        # Draw environmental effects with enhanced particle systems
        self.effects_manager.draw_arcade(
            arcade_window
        )  # Draw enhanced UI with survival statistics (Arcade version)
        game_stats = self.ui_manager.get_game_stats(
            self.snakes, self.food_items, self.effects_manager
        )
        self.ui_manager.draw_enhanced_hud_arcade(arcade_window, game_stats, self.snakes)

        # Draw enhanced snake info for critical snakes (Arcade version)
        for snake in self.snakes:
            self.ui_manager.draw_enhanced_snake_info_arcade(arcade_window, snake)
