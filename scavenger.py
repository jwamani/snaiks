...
class Scavenger:
    ...
    def find_target(self, food_items):
        ...
        if (
            current_time - self.last_target_update > 0.3
            or not self.target_food
            or self.target_food not in food_items
        ):
    ...
    def draw(self, screen):
        ...
        if speed > self.max_speed * 0.3:  # Wings out when moving
            ...
            left_wing = [
                ...
                (
                    int(self.position.x - self.size // 2),
                    int(self.position.y - self.size // 3),
                ),
                (
                    int(self.position.x - self.size // 2),
                    int(self.position.y + self.size // 3),
    ...
    def draw_arcade(self, arcade_window):
        """Draw scavenger using Arcade graphics with enhanced bird-like effects"""
        import arcade
        import math
        ...
        # Draw main body (crow-like)
        arcade.draw_circle_filled(
            self.position.x, self.position.y, self.size, self.body_color
        )

        # Draw animated wings (flapping effect)
        wing_flap = math.sin(time.time() * 10) * 0.3  # Flapping animation

        # Left wing
        arcade.draw_ellipse_filled(
            self.position.x - 8,
            self.position.y + wing_flap,
            self.wing_span // 2,
            self.size // 2,
            self.body_color,
            tilt_angle=20 + wing_flap * 30,
        )
        ...
        beak_end = self.position + direction * beak_length
        arcade.draw_line(
            self.position.x, self.position.y, beak_end.x, beak_end.y, self.beak_color, 3
        )

        # Draw bright yellow eyes
        eye_color = (255, 255, 100)
        arcade.draw_circle_filled(
            self.position.x - 2, self.position.y + 2, 1, eye_color
        )
        arcade.draw_circle_filled(
            self.position.x + 2, self.position.y + 2, 1, eye_color
        )

        # Draw target line if hunting food (subtle)
        if self.target_food:
            arcade.draw_line(
                self.position.x,
                self.position.y,
                self.target_food.position.x,
                self.target_food.position.y,
                (100, 100, 150, 128),
                1,
            )
