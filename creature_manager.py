...
class CreatureManager:
    ...
    def _update_rippers(self, snakes, current_time):
        ...
        if (
            hunter_percentage >= HUNTER_POPULATION_THRESHOLD
            and len(self.rippers) < MAX_RIPPERS_ON_SCREEN
            and current_time - self.last_ripper_check_time > RIPPER_SPAWN_INTERVAL
        ):
            self._spawn_ripper()
    ...
    def _update_scavengers(self, snakes, food_items, current_time):
        ...
        if (
            len(food_items) >= FOOD_ABUNDANCE_THRESHOLD
            and len(self.scavengers) < MAX_SCAVENGERS_ON_SCREEN
            and current_time - self.last_scavenger_check_time > SCAVENGER_SPAWN_INTERVAL
        ):
    ...
    def _spawn_ripper(self):
        ...
        print(
            f"Ripper {new_ripper.id} spawned at ({x}, {y}) - Hunter population too high!"
        )
    ...
    def _spawn_scavenger(self):
        ...
        print(
            f"Scavenger {new_scavenger.id} spawned at ({x}, {y}) - Abundant food detected!"
        )
    ...
    def draw_arcade(self, arcade_window):
        """Draw all creatures using Arcade graphics"""
        # Draw rippers if enabled
        if ENABLE_RIPPERS:
            for ripper in self.rippers:
                if not ripper.is_dead:
                    ripper.draw_arcade(arcade_window)

        # Draw scavengers if enabled
        if ENABLE_SCAVENGERS:
            for scavenger in self.scavengers:
                if not scavenger.is_dead:
                    scavenger.draw_arcade(arcade_window)

        # Draw guardians if enabled (future implementation)
        if ENABLE_GUARDIANS:
            for guardian in self.guardians:
                if not guardian.is_dead:
                    guardian.draw_arcade(arcade_window)
